 ...
 --SP_RPT_WeeklyHuddleReport2 '2023-09-10'
CREATE PROCEDURE [dbo].[SP_RPT_WeeklyHuddleReport2]
...
where StatusWEDate = @CountryWeDate and A.Status = 'SubmissionDate' and A.IsDeleted = 0  
UNION ALL
SELECT DISTINCT BadgeID FROM Appco360_PROD..Txn_SalesImport WHERE Country = 'MY' and WEDate = @CountryWeDate and IsDeleted = 0
) A
...
SELECT distinct CONVERT(DATE, SubmitDate, 103) as 'SubDate', NewIndiaDB_PROD.dbo.WeekedingDate(CONVERT(DATE, SubmitDate, 103)) as 'WeDate' , LeaDID, UserEmpCode, Amount
INTO #RawLead
...
SELECT @UnSuccessfulEarning = SUM(CAST(A.Amount * 12 * AI.PayoutPerc / 100.00 as decimal(18,2)))  FROM #RawLead A INNER JOIN #UnSuccessFul B ON A.LeadId = B.SerialNumber
...
UNION ALL
SELECT DISTINCT BADGEID  as 'BadgeNo'  FROM Appco360_PROD..[Txn_SalesImport] A where A.Country = 'HK' and A.SignUpPieces > 0 and A.Campaign in ('Viospace','Home Cuisine') and A.IsDeleted = 0 and A.WEDate = @CountryWeDate
) A
...
INSERT INTO #ReportDate (Country, Division, Customer, CampaignID, Weekending, RMonth, RQuarter, CountryUSHC, USCH)
SELECT 'Hong Kong' as 'Country', A.Division, A.Campaign as 'Customer', ID, @CountryWeDate as 'Weekending', @Month as 'Month' ,@Quarter as 'Quarter', @UniqueSCH as 'CountryUniqueSHC',  A.BadgeNo as 'UniqueSHC' FROM (
SELECT 'Hong Kong' as 'Country','Commercial' as 'Division' , A.Campaign, B.ID ,COUNT( distinct BadgeID) as 'BadgeNo'  
...
IsDeleted = 0 and WEDate = @CountryWeDate
...
A.NetPiece = B.Net, A.GrossSales = B.GrossEarning, A.GrossBAEarning = B.GrossEarning, A.NetBAEarning = B.NetEarning  FROM #ReportDate A INNER JOIN 
 ...
 SUM(Totalearning) as 'GrossEarning', SUM(NetEarning) as 'NetEarning'  FROM Appco360_PROD..[Txn_SalesImport]
...
where a.IsDeleted = 0 and MoSubmissionWEDate = '2023-09-24'
...
INSERT INTO #DailySHC
SELECT 'Korea','Charity', A.Campaign, A.MOSubDate, COUNT(DISTINCT FRID) from Appco360_PROD..SSIS_Maintable A
...
from Appco360_PROD..SSIS_Maintable  A LEFT JOIN NewOlaf_Prod..Mst_Campaign B ON B.CountryCode = 'KR' and A.Campaign = B.CampaignName collate SQL_Latin1_General_CP1_CI_AS 
...
-- Calculate Net Piece
SELECT distinct A.Campaign,'Charity' as Division,  0 as 'NetPiece', 0 as 'GrossPiece', 0 as 'GrossPieceOT', 0 as 'RejectPiece', 0 as 'ResubPiece', 0 as 'RejectPieceOT', 0 as 'RejectPieceREC', 0 as 'ResubPieceOT', 0 as 'ResubPieceREC' INTO #TempKoreaNet from Appco360_PROD..SSIS_Maintable A 
