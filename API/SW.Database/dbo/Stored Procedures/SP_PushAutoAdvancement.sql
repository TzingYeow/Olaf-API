...
CREATE PROCEDURE [dbo].[SP_PushAutoAdvancement]
...
--DECLARE @CurrentLevel INT = 4
 ...
 PRINT '1'
	SELECT
	@ReportBadgeEmail = ISNULL( (SELECT b.email FROM Mst_IndependentContractor b  LEFT JOIN Mst_MarketingCompany C ON B.MarketingCompanyId = C.MarketingCompanyId  WHERE b.BadgeNo = i.ReportBadgeNo and c.CountryCode = @CountryCode ),''),-- and b.MarketingCompanyId = i.MarketingCompanyId), 
	...
	BEGIN
		...
		DECLARE @EmailContent nvarchar(3000)
		...
		END
		ELSE IF @CurrentLevel = 7 and @CountryCode not in ( 'MY','TW','TH','HK','KR','ID') -- SENIOR TEAM LEADER
		BEGIN
			SET @TextTag =  'EmailAutoAdvancementSTL'
		END
		ELSE IF @CurrentLevel = 7 and @CountryCode in ( 'MY','TW','TH','HK','KR','ID') -- SENIOR TEAM LEADER
		BEGIN
		...
		BEGIN
			SET @TextTag =  'EmailAutoAdvancementAO'
		END
		ELSE IF @CurrentLevel = 2	-- OWNER PARTNER
		BEGIN
			SET @TextTag =  'EmailAutoAdvancementO'
		END
		--PRINT @TextTag
		...
		When 'ID' Then (SELECT IDDescription FROM Mst_Localization WHERE TextTag = @TextTag AND TextType = 'FullContent')
		...
		When 'ID' Then (SELECT IDDescription FROM Mst_Localization WHERE TextTag = @TextTag AND TextType = 'Subject')
		Else (SELECT ENDescription FROM Mst_Localization WHERE TextTag = @TextTag AND TextType = 'Subject')
		End 

		-- Implement Indonesia advancement. Email template for TL and AOP need to include contractor name at the email subject CR2024011213000273
		-- Advancement WE28012024 start
		 --SELECT @EmailSubject = REPLACE(@EmailSubject, '[CONTRACTORFULLNAME]', @ICFullName)
		...
		--CONCAT(@ReportBadgeEmail, ';', @OwnerEmail, ';', @EmailReceiver, ';' ,@AdminEmail + ';adilla.aziz@salesworks.asia;'), 'adilla.aziz@salesworks.asia', 
		CONCAT(@OwnerEmail, ';', @EmailReceiver, ';' ,@AdminEmail + ';adilla.aziz@salesworks.asia;'), 'adilla.aziz@salesworks.asia', 
