-- =============================================
 -- RPT_PRIncentive_Historical  'MY',''
-- =============================================
create PROCEDURE [dbo].[RPT_PRIncentive_Historical_SAM]
	 @Country NVARCHAR(50), @MCCode NVARCHAR(50)
AS
BEGIN
	...
	B.MilestoneHitWE, DATEADD(day, 7 , B.MilestoneHitWE) as 'PayWe', CASE WHEN B.MilestonesType = 1 THEN '' ELSE B.MilestonesData END as 'PRPoint',
	...
	LEFT JOIN MST_PR_CustomBankInfo BI ON SUBSTRING(RC.BadgeNo,1,2) = BI.MCCode and RC.CountryCode = BI.CountryCode

	WHERE   IC.Code not in ('NA') and A.IsDeleted = 0 and b.IsDeleted = 0
