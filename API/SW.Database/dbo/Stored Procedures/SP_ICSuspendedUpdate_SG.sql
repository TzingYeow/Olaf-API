...
CREATE PROCEDURE [dbo].[SP_ICSuspendedUpdate_SG]  
...
BEGIN
	...
	INNER JOIN Mst_MarketingCompany m ON m.MarketingCompanyId = B.MarketingCompanyId and m.CountryCode = 'SG'
	ON A.IndependentContractorId = B.IndependentContractorId WHERE A.StartDate = CONVERT(DATE, GETDATE()) AND B.IsSuspended = 0 AND A.IsDeleted = 0 AND B.Status = 'Active'

	UPDATE Mst_IndependentContractor SET IsSuspended = 1, UpdatedBy = 'ICSuspendedUpdate_SG', UpdatedDate = GETDATE()
	...
	BEGIN
		...
		FROM (
		SELECT s.IndependentContractorSuspensionId, i.IndependentContractorId, i.IndependentContractorLevelId			-- refer Ticket#2021062113000047 add 70 days for SG 
		...
		INNER JOIN Mst_MarketingCompany m ON m.MarketingCompanyId = i.MarketingCompanyId and m.CountryCode = 'SG'
		WHERE s.IsDeleted = 0 AND i.IsSuspended = 0 AND IsReactivate = 1 AND s.Description = 'No new sale in 3 week-endings' 
		AND EndDate <= FORMAT(DATEADD(DAY,-21, GETDATE()), 'yyyy-MM-dd') AND ISNULL(DATEADD(DAY,70, i.LastSalesSubmissionDate), '2021-03-26') < EndDate AND i.Status = 'Active' 
		) l
		...
		FROM Mst_IndependentContractor_Suspension s INNER JOIN Mst_IndependentContractor i ON s.IndependentContractorId = i.IndependentContractorId 
		INNER JOIN Mst_MarketingCompany o ON o.MarketingCompanyId = i.MarketingCompanyId and o.CountryCode = 'SG'
		...
		AND m.IndependentContractorMovementId is null 
		...
		WHERE CampaignId IN ( '1188', '1125', '1259') OR (CampaignId = '1205' AND MarketingCompanyId IN( '1492','1384'))  -- Ticket#2023112213000031
		AND a.IsDeleted = 0
		...
		--To fix issue second suspension within 180days/6months from first suspension
           ...
           ,[IsReactivate] ,[IsDeleted] ,[CreatedBy] ,[CreatedDate])
			...
			UPDATE d SET IsSuspended = 1, UpdatedBy = 'ICSuspendedUpdate_SG', UpdatedDate = GETDATE()
	...
	INNER JOIN Mst_MarketingCompany M ON M.MarketingCompanyId = B.MarketingCompanyId and M.CountryCode = 'SG'
	WHERE A.EndDate = CONVERT(DATE, DATEADD(DAY, -1, GETDATE())) AND B.IsSuspended = 1 AND B.Status = 'Active' AND A.IsReactivate = 0 AND A.IsDeleted = 0 
	...
	INNER JOIN Mst_IndependentContractor B ON A.IndependentContractorId = B.IndependentContractorId
	INNER JOIN Mst_MarketingCompany M ON M.MarketingCompanyId = B.MarketingCompanyId and M.CountryCode = 'SG'
	 WHERE A.EndDate = CONVERT(DATE, DATEADD(day, -1, GETDATE())) AND B.IsSuspended = 1 AND B.Status = 'Active' AND A.IsReactivate = 1 AND A.IsDeleted = 0
 
	UPDATE Mst_IndependentContractor SET IsSuspended = 0, UpdatedBy = 'ICSuspendedUpdate_SG', UpdatedDate = GETDATE()
	...
	WHERE (s.McAdmin is not null or ms.Email is not null) AND ms.IsDeleted = 0 AND m.IsDeleted = 0
