...
-- Description:	Recruitment Candidate Regional List
-- EXEC SP_RPT_RecruitmentCandidate_Regional 'b2a2a1706a0d403598e9115d5846485d','HK,KR,MY,TH,TW,SG' -- REGIONAL ADMIN
-- =============================================
CREATE PROCEDURE [dbo].[SP_RPT_RecruitmentCandidate_Regional]
(           
 @userId NVARCHAR(255),  
...
WHERE a.IsDeleted = 0 and a.Status != 'Draft' AND B.CountryCode IN (SELECT Country FROM #SubCountry)
Order by a.RecruitmentCandidateId desc

IF OBJECT_ID('tempdb..#InterviewDateTime') IS NOT NULL DROP TABLE #InterviewDateTime
SELECT RecruitmentCandidateId,MAX(ScheduleStartDateTime) AS 'InterviewDateTime' INTO #InterviewDateTime FROM Mst_RecruitmentCandidate_ActivityLog 
WHERE Stage = 'First-Appointment' and AppointmentStatus = 'Registered' and IsDeleted = 0 --AND ScheduleStartDateTime <  DATEADD(year,-2,GETDATE())
GROUP BY RecruitmentCandidateId ORDER BY InterviewDateTime DESC
...
INTO #InterviewTurnedUp FROM Mst_RecruitmentCandidate_ActivityLog WHERE Stage = 'First-Appointment' and AppointmentStatus = 'Turned-Up' and IsDeleted = 0 
GROUP BY RecruitmentCandidateId ORDER BY InterviewTurnedUpDateTime DESC
...
DELETE A		
FROM #MainData A	
INNER JOIN #InterviewDateTime B ON A.RecruitmentCandidateId = B.RecruitmentCandidateId
WHERE B.InterviewDateTime < DATEADD(year,-2,GETDATE())
	...
	ORDER BY CountryCode ASC, MarketingCompanyId ASC, InterviewDateTime ASC
END
