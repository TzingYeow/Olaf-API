...
-- SP_RPT_RCAnalysis_Regional_Detailed '2021-01-01','b2a2a1706a0d403598e9115d5846485d'
-- ==========================================================================================
CREATE PROCEDURE [dbo].[SP_RPT_RCAnalysis_Regional_Detailed]
...
-- ================== NO. OF EXITS (NON- LEADER)  ========================
		...
		--SELECT WeDate, Description, COUNT(*) from #DETAILED 
		--WHERE Description IN ('ORTotalActiveHeadcount','pRTotalActiveHeadcount','ORTotalActiveHCLeaders','PRTotalActiveHCLeaders')
		--GROUP BY WeDate, Description
		--order by WeDate ASC, Description ASC 

		--SELECT * from #DETAILED
		--WHERE Description IN ('ORActiveHeadcount')
		--and WeDate = '2020-11-05' order by description asc

		--SELECT COUNT(*) FROM #DETAILED WHERE Description = 'ORTotalActiveHeadcount' AND Country = 'my' AND MCCode = 'LO' AND Channel = 'F2F'
		--SELECT COUNT(*) FROM #DETAILED WHERE Description = 'PRTotalActiveHeadcount' AND Country = 'my' AND MCCode = 'LO' AND Channel = 'F2F'

-- ========================= PREP (END) ===========================================================================================================

		---- NULL RECRUITER
		--SELECT A.CountryCode as MCCountry,A.MCCode,A.WEStatus,A.RecruitmentType,B.RecruitmentSource AS Channel, B.RecruiterBadgeNoOrName AS Recruiter,
		--B.RecruitmentCandidateId,b.IndependentContractorId,B.BadgeNo
		--FROM TXN_ICWeekendingStatus A
		--INNER JOIN Mst_IndependentContractor B ON A.BAID = B.IndependentContractorId
		--WHERE WESTATUS >= @Mindate AND WESTATUS <= @Maxdate AND ACTIVESTATUS = 1
		--AND A.CountryCode  IN (SELECT Value from #CountryAcess)
		--AND b.MarketingCompanyId not in  (SELECT MarketingCompanyId FROM #MCExclusion)
		--AND MCCode = 'FS' AND RecruiterBadgeNoOrName IS NULL
		--ORDER BY MCCountry ASC,MCCode ASC,Channel ASC,RecruiterBadgeNoOrName ASC

		---- DUPLICATE BA IN INDUCTION
		--SELECT RecruitmentCandidateId,MCCountry,MCCode,convert(date,ActivityScheduleStartWeekending) as WeDate,RecruitmentSource As Channel,RecruiterBadgeNoOrName As Recruiter,RecruitmentType
		--FROM #maindata 
		--WHERE ActivityStage = 'induction'
		--AND MCCode = 'FT' AND MCCountry = 'SG' AND RecruitmentType = 'personal Recruitment'
		--ORDER BY RecruitmentCandidateId asc,WeDate asc , Recruiter asc

		---- EXTRA COUNT FROM Observation
		--SELECT IsDeleted,RecruitmentCandidateId,MCCountry,MCCode,convert(date,ActivityScheduleStartWeekending) as WeDate,RecruitmentSource As Channel,RecruiterBadgeNoOrName As Recruiter,RecruitmentType
		--FROM #maindata 
		--WHERE ActivityStage = 'Observation' AND ActivityAppointment = 'Registered' AND RecruiterBadgeNoOrName = 'Wong Fui San'
		--ORDER BY MCCountry ASC,MCCode ASC,WeDate ASC,Channel ASC,RecruiterBadgeNoOrName ASC, RecruitmentType ASC

		--SELECT * FROM #EXITSNL WHERE Recruiter = 'OZ00152' ORDER BY WEdate ASC

		--SELECT * from #DETAILED WHERE MCCode = 'oz' AND Recruiter = 'OZ00152' order by Country asc, MCCode asc, WeDate asc


--SP_RPT_RCAnalysis_Regional_Detailed '2020-09-01','b2a2a1706a0d403598e9115d5846485d'
		

--SELECT RecruitmentCandidateId,RecruitmentSource As Channel,RecruiterBadgeNoOrName As Recruiter,MCCountry,MCCode,convert(date,ActivityScheduleStartWeekending) as WeDate
--FROM #maindata 
--WHERE ActivityStage = 'First-Appointment' AND ActivityAppointment = 'Registered'
--and MCCode = 'ca' and MCCountry = 'sg' and RecruiterBadgeNoOrName = 'vanessa'
--order by WeDate asc

--select * from #FISRTAPPBOOKED where MCCode = 'ca' and MCCountry = 'sg'
--SELECT * from #DETAILED order by Country asc, MCCode asc, WeDate asc
