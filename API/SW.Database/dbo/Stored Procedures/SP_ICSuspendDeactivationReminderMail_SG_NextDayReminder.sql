...
-- NEXT DAY DEACTIVATION REMINDER (FOR WEDNESDAY CURRENT WEEK)
-- =============================================
CREATE PROCEDURE [dbo].[SP_ICSuspendDeactivationReminderMail_SG_NextDayReminder]  
...
BEGIN
	...
	SET NOCOUNT ON;
		...
		SET @SUSPENSIONRUNDATE = CAST(DATEADD(DAY, 1, GETDATE()) AS DATE) -- RUNDATE FOR SG IS EVERY WEDNESDAY
		...
		--To fix issue second suspension within 180days/6months from first suspension
			...
			-- TEMPTABLE DEACTIVATION LIST
			INSERT INTO [dbo].[Mst_Suspension_TempTable]
           ([Type] ,[SuspensionRunDate] ,[CountryCode] ,[MCCode] ,[BadgeNo] ,[LastSalesDate] ,[LastSalesSubmissionDate] ,[CORRECTEDLASTDATE]
           ,[IndependentContractorId] ,[Description] ,[IndependentContractorLevelId] ,[StartDate] ,[EndDate] ,[IsReactivate] ,[IsDeleted] ,[CreatedBy] ,[CreatedDate])
			SELECT 'Deactivation',@SUSPENSIONRUNDATE,CountryCode,MCCode,BadgeNo,LastSalesDate,LastSalesSubmissionDate,CORRECTEDLASTDATE
			,IndependentContractorId, 'Deactivate', IndependentContractorLevelId, NULL, NULL ,0, 0, 'DeactivationRun', @SUSPENSIONRUNDATE
			FROM #DeactivationList ORDER BY CountryCode, MCCode, BadgeNo ASC
