...
-- NEXT WEEK DEACTIVATION REMINDER (FOR WEDNESDAY NEXT WEEK)
-- =============================================
CREATE PROCEDURE [dbo].[SP_ICSuspendDeactivationReminderMail_SG]  
...
BEGIN
	...
	SET NOCOUNT ON;
		...
		SET @SUSPENSIONRUNDATE = CAST(DATEADD(DAY, 7, GETDATE()) AS DATE) -- RUN DAY IS WEDNESDAY THEN GET NEXT WEDNESDAY
		...
		WHERE D.BadgeNo NOT IN (SELECT BadgeNo FROM Mst_Suspension_TempTable WHERE Type = 'Deactivation')
