...
CREATE PROCEDURE SP_RPT_Adoption_OverviewRecruitmentQuater
...
BEGIN
	...
	INSERT INTO #TempRaw 
	SELECT DATEPART(QUARTER, WeDate) as 'WeDate', 'Q' + CAST(DATEPART(QUARTER, WeDate) as nvarchar) + ' ' +  CAST(YEAR(WEdate) as nvarchar) as 'Desc' , SUM(TotalMCActive) / COUNT(*) as 'AvgMCActive', SUM(TotalMC) / COUNT(*)  as 'AvgMc',0
	 FROM RPT_OverviewRecruitment WHERE YEAR(WeDate) = YEAR(GETDATE())
