...
-- SG NEXT DAY REMINDER (SUSPENSION RUN WEDNESDAY CURRENT WEEK)
-- =============================================
CREATE PROCEDURE [dbo].[SP_ICSuspendReminderMail_SG_NextDayReminder]  
...
BEGIN
	...
	SET NOCOUNT ON;
	 DECLARE @SUSPENSIONRUNDATE DATE;
		SET @SUSPENSIONRUNDATE = CAST(DATEADD(DAY, 1, GETDATE()) AS DATE) -- NEXTDAY REMINDER FOR SG IS FOR WEDNESDAY RUN
		...
		--To fix issue second suspension within 180days/6months from first suspension
			...
			--TempTable Suspension Details
			TRUNCATE TABLE Mst_Suspension_TempTable
           ...
           ,[IsReactivate] ,[IsDeleted] ,[CreatedBy] ,[CreatedDate])
			SELECT 'Suspension',@SUSPENSIONRUNDATE,CountryCode,MCCode,BadgeNo,LastSalesDate,LastSalesSubmissionDate,CORRECTEDLASTDATE
			...
			,0,0,'SuspensionRun',@SUSPENSIONRUNDATE
			FROM #SuspensionList ORDER BY CountryCode, MCCode, BadgeNo ASC
 ...
 -- EMAIL REGION (START)===================================================================================================================================================================================================
	...
	FORMAT (@SUSPENSIONRUNDATE, 'dd-MM-yyyy'), '</b> due to No Sales in 3 consecutive weekendings if they did not submit any new sales within the next day.<br>Please contact your country PIC for further clarification.<br>Thanks.<br><br>', REPLACE(Badge, ';', '<br>'), '</span>') 
	, GETDATE(), 'Admin' 
	...
	'OLAF Suspension List Reminder: No sales for 3 consecutive Weekending', 
	CONCAT('<span style="font-size:10pt;font-family:Helvetica">Hi,<br>Kindly take note that below ID badges will be suspended on <b>', 
	FORMAT(@SUSPENSIONRUNDATE, 'dd-MM-yyyy'),'</b> due to No Sales in 3 consecutive weekendings if they did not submit any new sales within the next day.<br>Please contact your country PIC for further clarification.<br>Thanks.<br>', REPLACE(Badge, ';', '<br>'), '</span>') 
	, GETDATE(), 'Admin' 
