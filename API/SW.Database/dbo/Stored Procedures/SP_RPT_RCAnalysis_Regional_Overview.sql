...
-- EXEC SP_RPT_RCAnalysis_Regional_Overview '2022-08-23','b2a2a1706a0d403598e9115d5846485d'
-- ==========================================================================================
CREATE PROCEDURE [dbo].[SP_RPT_RCAnalysis_Regional_Overview]
...
-- ========================= Define Time Range (START) ================================================================================================
		...
		IF @Searchmonth = @Currentmonth
			BEGIN
				...
				-- ADDED ON 03/05
				Select @MAXDATE = ToDate FROM #Weekending WHERE NoOfWE = @NUMOFWE ORDER BY ToDate DESC			END
		ELSE
			BEGIN
...
-- ================== INACTIVE BA  ========================
		...
		SELECT DISTINCT MCCountry,MCID,IndependentContractorId, IndependentContractorLevelId, RecruitmentType, Description, max(EffectiveDate) AS EffectiveDate 
		INTO #INACTIVEIC
