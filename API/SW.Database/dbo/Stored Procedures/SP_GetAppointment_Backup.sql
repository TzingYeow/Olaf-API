...
CREATE PROCEDURE [dbo].[SP_GetAppointment_Backup]
AS
BEGIN
	SET NOCOUNT ON;
...
SELECT RecruitmentCandidateId FROM #RawList
 ...
 order by A.RecruitmentCandidateId
  ...
  SELECT A.RecruitmentCandidateId, A.MCCountry, A.ActivityScheduleStartDateTime, A.MCCode, A.MCName, A.FirstName, A.LastName, A.Email,  Case WHEN C.RecruitmentCandidateId IS NULL THEN 'NO' ELSE 'YES' END as '	Created', Case WHEN B.RecruitmentCandidateId IS NULL THEN 'NO' ELSE 'YES' END as 'Submitted', 'YES' as 'First Appointment' 
...
where A.ActivityStage = 'First-Appointment' and A.ActivityAppointment ='Turned-Up'  and A.ActivityScheduleStartWeekending = '2021-02-04'
