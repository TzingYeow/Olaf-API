...
-- [SP_RPT_RCAnalysis_Regional_Detailed_IC_SAM] '2022-12-01','b2a2a1706a0d403598e9115d5846485d'
-- [SP_RPT_RCAnalysis_Regional_Detailed_IC_SAM] '2022-11-01','b2a2a1706a0d403598e9115d5846485d'
-- ==========================================================================================
CREATE PROCEDURE [dbo].[SP_RPT_RCAnalysis_Regional_Detailed_IC_SAM]
	-- Add the parameters for the stored procedure here
	@Searchdate  date,
	@userId varchar(150)
AS
BEGIN
	--SELECT * FROM MST_USER WHERE UserId = 'b2a2a1706a0d403598e9115d5846485d'
...
-- ========================= Define Time Range (START) ================================================================================================
		...
		IF (@Searchmonth = @Currentmonth AND @Searchyear = Year(@Todaydate))
			BEGIN
		...
		ELSE
			...
			END 
				...
				SELECT @SEARCHMONTH AS SEARCHMONTH, @CURRENTMONTH AS CURRENTMONTH, @Searchyear Searchyear
...
-- ================== NO. OF EXITS (LEADER)  ========================
		...
		WHERE Description IN ('Deactivate','Deactivated','Terminate') AND IndependentContractorLevelId= '4' and IsDeleted = 0
...
-- ================== NO. OF EXITS (NON- LEADER)  ========================
		...
		WHERE Description IN ('Deactivate','Deactivated','Terminate') AND IndependentContractorLevelId not in( '4' ) and IsDeleted = 0
		GROUP BY MCCountry,MCCode,WeDate,RecruitmentSource,RecruiterBadgeNoOrName,RecruitmentType
		ORDER BY MCCountry ASC,MCCode ASC,WeDate ASC,Channel ASC,RecruiterBadgeNoOrName ASC
		...
		--SELECT DISTINCT Description from #DETAILED WHERE WeDate <= @Maxdate 

				SELECT Country, MCCode , 'OCT-22' Month, Description, SUM(Total)Total  
				FROM #DETAILED WHERE WeDate <= @Maxdate 
				AND Description IN ('ORTotalActiveHeadcount','PRTotalActiveHeadcount')
				and Country IN ('HK','MY','SG','TW')
				GROUP BY Country, MCCode, Description
				order by Country asc, MCCode asc

		-- [SP_RPT_RCAnalysis_Regional_Detailed_IC_SAM] '2022-11-01','b2a2a1706a0d403598e9115d5846485d'
		-- [SP_RPT_RCAnalysis_Regional_Detailed_IC_SAM] '2023-10-01','b2a2a1706a0d403598e9115d5846485d'
