...
-- EXEC [SP_RPT_SalesDuringSuspension_OtherAccess] '2023-01-08','2023-01-08','ba608c0d064243d88450d8c948e4df40'-- MO ADMIN
-- =============================================
CREATE PROCEDURE [dbo].[SP_RPT_SalesDuringSuspension_OtherAccess]
	 @weFromDate Date,
	 @weToDate Date,
	 @userId NVARCHAR(255)
...
WHERE A.IsDeleted = 0 AND Status IN ( 'SubmissionDate' , 'ReSubmissionDate', 'ClientReSubmissionDate')  AND StatusWEDate >=  @weFromDate AND StatusWEDate <= @weToDate
...
WHERE A.IsDeleted = 0 AND A.Status IN ( 'SubmissionDate' , 'ReSubmissionDate', 'ClientReSubmissionDate')  AND StatusWEDate >=  @weFromDate AND StatusWEDate <= @weToDate
-- SINGAPORE (END)
...
WHERE A.IsDeleted = 0 AND A.Status IN ( 'SubmissionDate' , 'ReSubmissionDate') AND StatusWEDate >=  @weFromDate AND StatusWEDate <= @weToDate
...
WHERE A.Country = 'HK' and A.SignUpPieces > 0 and A.Campaign in ('Viospace','Home Cuisine') and IsDeleted = 0AND wedate >=  @weFromDate AND wedate <= @weToDate
-- HONG KONG (END)
...
where A.IsDeleted = 0 AND MoSubmissionWEDate >=  @weFromDate AND MoSubmissionWEDate <= @weToDate
-- THAILAND (END)
...
WHERE MOSubWE >=  @weFromDate AND MOSubWE <= @weToDate
-- KOREA (END) 
...
IF @IsMOAdminRole = 1
	BEGIN
	SELECT 'ADMIN' AS 'Role',* FROM #FinalReport A WHERE A.MOID = @MCId
	END
ELSE
	BEGIN
	SELECT 'NON ADMIN' AS 'Role',* FROM #FinalReport A WHERE A.CountryCode = @CountryCode
	END
