...
--=============================
CREATE PROCEDURE [dbo].[SP_RPT_WeeklyHuddleReport_BulletinPoint]
...
GROUP BY MOCode
UNION  
SELECT OfficeId_H FROM NewMYDB_PROD..Tbl_TKF_Maintable_StatusSummary WHERE StatusWE = @CountryWeDate and IsDeleted = 0 and Status in ('SubmissionDate','ReSubDate','ClientReSubDate','ClientRejectDate','RejectDate') 
UNION ALL
...
where StatusWEDate = @CountryWeDate  and A.Status in ('SubmissionDate','ReSubmissionDate','ClientReSubmissionDate','UpDownResubDate','ClientRejectDate','UpDownRejectDate','RejectDate') and A.IsDeleted = 0   GROUP BY MOCode
UNION 
...
UNION 
SELECT DISTINCT SUBSTRING(Badgeid,1,2)
...
SELECT SUBSTRING(Badgeid,1,2) as 'MOCode', SUM(totalearning) as 'Gross', SUM(Netearning) as 'netSales'
FROM Appco360_PROD..Txn_SalesImport A INNER JOIN Mst_Campaign B ON A.Campaign = B.CampaignCode 
LEFT JOIN Mst_Division C ON B.DivisionId = C.DivisionId
WHERE Country = 'MY' and WEDate =@CountryWeDate and A.IsDeleted = 0 and 
ISNULL(TotalEarning,0.00) > 0.00 and ISNULL(SignUpPieces,0) > 0
GROUP BY  SUBSTRING(Badgeid,1,2)
) B ON A.Country = 'Malaysia' and A.MOCode = B.MOCode
...
UPDATE A SET A.SWBonus = ISNULL(A.SWBonus,0.00) + ISNULL(B.Bonus,0.00) FROM #ReportDate A INNER JOIN (
SELECT SUBSTRING(Badgeid,1,2) as 'OfficeID', SUM(SWBonus) as 'Bonus' 
FROM Appco360_PROD..Txn_SalesImport A LEFT JOIN Mst_Campaign B ON A.Campaign = B.CampaignCode 
INNER JOIN Mst_Division C ON B.DivisionId = C.DivisionId
WHERE Country = 'MY' and WEDate = @CountryWeDate and A.IsDeleted = 0 and 
ISNULL(TotalEarning,0.00) > 0.00 and ISNULL(SignUpPieces,0) > 0
GROUP BY  SUBSTRING(Badgeid,1,2)
) B ON A.MOCode = B.OfficeId and A.Country = 'Malaysia'
...
SELECT DISTINCT BadgeNo FROM #StarHubTempResult WHERE  GrossQty > 0  and Mocode not in ('HQ') 
) A
...
INSERT INTO #ReportDate (Country, MOCode, MOName, Weekending, RMonth, RQuarter, GrossBAEarning, NetBAEarning, SWBonus )
SELECT 'Singapore' as 'Country', A.MOCode, A.Name , @CountryWeDate as 'Weekending', @Month as 'Month' ,@Quarter as 'Quarter',0.00,0.00,0.00 FROM (
SELECT DISTINCT A.MOCode, B.Name FROM #StarHubTempResult A LEFT JOIN NewSGDB_PROD..VW_MST_MO B ON A.MOCode = B.MOCode WHERE  GrossQty > 0  and A.Mocode not in ('HQ') 
and ISNULL(BadgeNo,'') <> ''  
) A LEFT JOIN #ReportDate Z ON A.MOCode = Z.MOCode  and Z.Country = 'Singapore'
  WHERE Z.MOCode is null 
...
UPDATE A SET A.GrossBAEarning = B.gross, A.NetBAEarning = B.NetSales  FROM #ReportDate A INNER JOIN(
SELECT A.MOCode ,SUM(A.GrossBAFees) as 'Gross', 
SUM(A.GrossBAFees + A.ResubBAFees - A.RejectBAFees) as 'NetSales' 
FROM #StarHubTempResult A WHERE  GrossQty > 0  and A.Mocode not in ('HQ')
GROUP BY A.MOCode 
) B ON A.Country = 'Singapore' and A.MOCode = B.MOCode
...
INSERT INTO #ReportDate (Country, MOCode, MOName, Weekending, RMonth, RQuarter, GrossBAEarning, NetBAEarning, SWBonus )
SELECT 'India' as 'Country', A.MOCode, A.Name , @CountryWeDate as 'Weekending', @Month as 'Month' ,@Quarter as 'Quarter',0.00,0.00,0.00 FROM (
SELECT DISTINCT A.MOCode, B.Name FROM NewIndiaDB_PROD..VW_CH_SS A LEFT JOIN NewIndiaDB_PROD..VW_MST_MO B ON A.MOCode = B.MOCode WHERE StatusWEDate = @CountryWeDate 
and ISNULL(BadgeNo,'') <> '' and Status = 'SubmissionDate' and IsDeleted = 0  
) A 
...
SELECT DISTINCT A.MOCode, B.Name FROM NewIndiaDB_PROD..VW_CH_SS A LEFT JOIN NewIndiaDB_PROD..VW_MST_MO B ON A.MOCode = B.MOCode WHERE StatusWEDate = @CountryWeDate 
and ISNULL(BadgeNo,'') <> '' and Status <> 'SubmissionDate' and IsDeleted = 0 ) A  
LEFT JOIN #ReportDate Z ON A.MOCode = Z.MOCode  and Z.Country = 'India'
WHERE Z.MOCode is null 
      
UPDATE A SET A.GrossBAEarning = B.gross, A.NetBAEarning = B.NetSales  FROM #ReportDate A INNER JOIN(
SELECT A.MOCode ,SUM(CASE WHEN A.Status in ('AgencyApproved') THEN AgencyInvoiceValue ELSE 0 END) as 'Gross', 
SUM(CASE WHEN Status in ('AgencyApproved') THEN AgencyInvoiceValue WHEN status in ('AgencyReject') THEN AgencyInvoiceValue * -1.00 ELSE 0 END) as 'NetSales' 
FROM NewIndiaDB_PROD..VW_CH_SS A  
  WHERE StatusWEDate = @CountryWeDate -- and ICStrokeValue > 0
and ISNULL(BadgeNo,'') <> '' and A.Status in ('AgencyReject','AgencyApproved','SubmissionDate','ReSubmissionDate','ClientResubmissionDate','RejectDate','ClientRejectDate')
and A.IsDeleted = 0
GROUP BY A.MOCode 
) B ON A.Country = 'India' and A.MOCode = B.MOCode
...
-- to handle where MO only has reject without any submission
INSERT INTO #ReportDate (Country, MOCode, MOName, Weekending, RMonth, RQuarter, GrossBAEarning, NetBAEarning, SWBonus ) 
SELECT   'Korea' as 'Country', A.OfficeID, A.OfficeName,  @weDate, @Month, @Quarter, 0,0,0.00
from Appco360_PROD..SSIS_Maintable  A 
LEFT JOIN NewOlaf_Prod..Mst_Campaign B ON B.CountryCode = 'KR' and A.Campaign = B.CampaignName collate SQL_Latin1_General_CP1_CI_AS
LEFT JOIN #ReportDate gross
on gross.Country = 'Korea' and gross.Weekending = A.MORejectWE and A.OfficeID = gross.MOCode collate SQL_Latin1_General_CP1_CI_AS
WHERE MORejectWE = @weDate and OfficeID <> 'HQ'  and PaidAmount > 0 and gross.MOCode is null
GROUP BY  A.OfficeID, A.OfficeName
...
SELECT 'Hong Kong' as 'Country', MOCode, Name, @CountryWeDate as 'Weekending', @Month as 'Month' ,@Quarter as 'Quarter',Gross,NetEarnings,SWBonus FROM (
 ...
 SELECT MOCODE, Name, SUM(Gross)[Gross], SUM(NetEarnings)[NetEarnings], SUM(SWBonus)[SWBonus]
 FROM (
 ...
 CAST(0 as decimal(18,2)) as 'Gross',
 ...
 UNION ALL
 SELECT B.OfficeCode as 'MOCODE', C.Name, CAST(SUM(TotalEarning) as decimal(18,2)) as 'Gross', CAST(SUM(ISNULL(NetEarning,0.00)  + ISNULL(BOND,0.00)) as decimal(18,2)) as 'NetEarnings', CAST(SUM(SWBonus) as decimal(18,2)) as 'SWBonus'  
 FROM Appco360_PROD..[Txn_SalesImport] A
 INNER JOIN NewHKDB_PROD..VW_MST_IC B ON B.BadgeNo = A.BadgeID
 LEFT JOIN NewOlaf_Prod..Mst_MarketingCompany C ON B.OfficeCode = C.Code and C.CountryCode = 'HK'
 WHERE Country = 'HK' and WEDate = @CountryWeDate and A.IsDeleted = 0 --and A.SignUpPieces > 0
 GROUP BY B.OfficeCode  , C.Name 

 ) COMBINE_CH_CO
 GROUP BY MOCODE, Name
 
) A  
 
-- Gross Earning
UPDATE A SET GrossBAEarning += ISNULL(B.Gross,0)
FROM #ReportDate A
...
SELECT 'Thailand' as 'Country', A.MOCode, B.Name, @CountryWeDate as 'Weekending', @Month as 'Month' ,@Quarter as 'Quarter', Gross,NetEarning,Bonus FROM (
 ...
 SELECT MOCode, SUM(ISNULL(SubmissionEarning,0.00) + ISNULL(ResubEarning,0.00) - ISNULL(SubmissionADBKKEarning2,0.00) + ISNULL(ValidADBKKEarning,0.00)) as 'GrossEarning',SUM(ISNULL(NetEarnings,0.00)) as 'NetEarning', SUM(ISNULL(AppcoBonus,0.00)) as 'Bonus' 
...
DELETE FROM #ReportDate WHERE ISNULL(GrossBAEarning,0.00) = 0.00 and ISNULL(NetBAEarning,0.00) = 0.00 and ISNULL(SWBonus,0.00) = 0.00
...
SELECT * FROM #ReportDate --WHERE Country <> 'India'
