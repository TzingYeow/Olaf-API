...
-- =============================================
CREATE PROCEDURE [dbo].[SP_ICSuspendReminderMail_LEO]  
...
BEGIN
	...
	FORMAT (DATEADD(wk, DATEDIFF(wk, -5, GETDATE()),4), 'dd-MM-yyyy'), '</b> due to No Sales in 3 consecutive weekendings if they did not submit any new sales within the next 7 days.<br>Please contact your country PIC for further clarification.<br>Thanks.<br><br>', REPLACE(Badge, ';', '<br>'), '</span>') , GETDATE(), 'Admin' 
	...
	'OLAF Suspension List Reminder: No sales for 3 consecutive Weekending', 
	CONCAT('<span style="font-size:10pt;font-family:Helvetica">Hi,<br>Kindly take note that below ID badges will be suspended on <b>', 
	FORMAT(DATEADD(wk, DATEDIFF(wk, -5, GETDATE()),4), 'dd-MM-yyyy'),'</b> due to No Sales in 3 consecutive weekendings if they did not submit any new sales within the next 7 days.<br>Please contact your country PIC for further clarification.<br>Thanks.<br>', REPLACE(Badge, ';', '<br>'), '</span>') 
	, GETDATE(), 'Admin' 
