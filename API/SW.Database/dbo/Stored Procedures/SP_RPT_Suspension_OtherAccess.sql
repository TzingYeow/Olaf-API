...
-- EXEC [SP_RPT_Suspension_OtherAccess] '2023-01-08','2023-01-08','ba608c0d064243d88450d8c948e4df40'-- MO ADMIN
-- =============================================
CREATE PROCEDURE [dbo].[SP_RPT_Suspension_OtherAccess]
	 @weFromDate Date,
	 @weToDate Date,
	 @userId NVARCHAR(255)
...
WHERE IsDeleted = 0 AND Status IN ( 'SubmissionDate' , 'ReSubmissionDate', 'ClientReSubmissionDate') AND StatusWEDate IN ( @weFromDate,  @weToDate)
AND MOCode <> NULL
...
WHERE A.IsDeleted = 0 AND Status IN ( 'SubmissionDate' , 'ReSubmissionDate', 'ClientReSubmissionDate') AND A.StatusWE IN ( @weFromDate,  @weToDate)
...
WHERE A.IsDeleted = 0 AND Status IN ( 'SubmissionDate' , 'ReSubmissionDate', 'ClientReSubmissionDate') AND A.StatusWEDate IN ( @weFromDate,  @weToDate)
...
WHERE IsDeleted = 0 AND MO_Sub_Week IN ( @weFromDate , @weToDate)
...
WHERE IsDeleted = 0 AND WEdate IN ( @weFromDate,  @weToDate)
-- MALAYSIA (END)
...
WHERE A.IsDeleted = 0 AND A.Status IN ( 'SubmissionDate' , 'ReSubmissionDate', 'ClientReSubmissionDate') AND A.StatusWEDate IN ( @weFromDate,  @weToDate)
-- SINGAPORE (END)
...
CASE WHEN A.Status IN ('SubmissionDate','ReSubmissionDate') THEN 'PENDING' WHEN A.Status IN ('ClientRejectDate','RejectDate','OfficeRejectDate') THEN 'REJECT' WHEN A.Status IN ('ClientApproved','AutoApproveDate') THEN 'APPROVED' END AS SNStatus
FROM NewHKDB_PROD..Txn_Transaction_StatusSummary  A LEFT JOIN NewHKDB_PROD..VW_CHR_TXN B ON A.TXNID = B.TXNID
WHERE A.IsDeleted = 0 AND A.Status IN ( 'SubmissionDate' , 'ReSubmissionDate') AND A.StatusWEDate IN ( @weFromDate,  @weToDate)
...
WHERE A.Country = 'HK' and A.SignUpPieces > 0 and A.Campaign in ('Viospace','Home Cuisine') and IsDeleted = 0 AND A.wedate IN ( @weFromDate,  @weToDate)
-- HONG KONG (END)
...
where A.IsDeleted = 0 and MoSubmissionWEDate IN ( @weFromDate,  @weToDate)
-- THAILAND (END)
...
WHERE MOSubWE IN ( @weFromDate,  @weToDate) 
-- KOREA (END) 
...
IF @IsMOAdminRole = 1
	BEGIN
	SELECT 'ASMIN',* FROM #FinalReport A WHERE A.MOID = @MCId
	END
ELSE
	BEGIN
	SELECT 'NON ADMIN',* FROM #FinalReport A WHERE A.CountryCode = @CountryCode
	SELECT * FROM #ReportDate A WHERE A.CountryCode = @CountryCode
	END
...
--SELECT * FROM Mst_MarketingCompany WHERE CODE = 'UL'
